...
class UNetConvBlock(nn.Cell):
    ...
    def construct(self, x):
        ...
        out = self.conv_1(x)
        if self.use_HIN:
            # out_1, out_2 = ops.split(out, out.shape[1]//2, axis=1)
...
class HighNet(nn.Cell):
    def __init__(self, nc):
        ...
        self.convout = nn.Conv2d(3, 3, 3, 1, pad_mode='pad',padding=1,has_bias=True)
        self.trans = nn.Conv2d(6,32,1,1,has_bias=True)
        self.con_temp1 = nn.Conv2d(32,32,3,1,pad_mode='pad',padding=1,has_bias=True)
        self.con_temp2 = nn.Conv2d(32,32,3,pad_mode='pad',padding=1,has_bias=True)
        self.con_temp3 = nn.Conv2d(32,3,3,1,pad_mode='pad',padding=1,has_bias=True)
    ...
    def construct(self,x, y_down, y_down_amp, y_down_phase):
        ...
        y_aff = self.trans(ops.concat([ops.interpolate(y_down, mode='bilinear',size=(int(y_down.shape[2]*2), int(y_down.shape[3]*2))), xout_temp], 1))
...
class LowNet(nn.Cell):
    def __init__(self, in_nc, nc):
        ...
        self.convout = nn.Conv2d(nc,12,1,1,has_bias=True)
        self.convoutfinal = nn.Conv2d(12, 3, 1, 1, has_bias=True)

        self.transamp = nn.Conv2d(12,12,1,1,has_bias=True)
        self.transpha = nn.Conv2d(12,12, 1, 1,has_bias=True)
...
class InteractNet(nn.Cell):
    ...
    def construct(self, x):
        ...
        x_f_down = ops.interpolate(x_f,mode='bilinear',size=(int(x_f.shape[2]*0.5), int(x_f.shape[3]*0.5)))
